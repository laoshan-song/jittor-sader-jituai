...
DATA = os.environ.get(
    "DATA_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "data_A.zip"),
)
if not os.path.exists(DATA):
    DATA = "data_A.zip"  # fallback to original path
BASE = 1 << 32
RAW_DIM = 22
SRC_FREQ_DIM = 24
HIST_LEN = 8
...
# dataset2: 0.15 -> 0.02 -> 0.0 -> 0.02 -> 0.0 (pure base outperforms blend after retraining)
PROP_ALPHA = {"dataset1": 0.10, "dataset2": 0.05}
# Second-stage ranker blend weight. Tuned on held-out val: dataset2 ranker hurts after
# base model improvement, so blend=0.0 (pure base). Re-tune after every retrain.
RANK_BLEND = {"dataset2": 0.0}
# Transductive candidate-pool frequency bonus. Tuned on held-out val (20k samples):
# dataset1: 0.0, dataset2: 0.06 (optimum found at freq_bonus=0.06 in grid search).
FREQ_BONUS = {"dataset1": 0.0, "dataset2": 0.0}
# Temporal-decay popularity blend. dataset2 is a cold-start recommendation problem where
# the answer is always a NEW item, so recent global popularity trend is a strong signal
# (single-signal fair-MRR 0.42 vs static popularity 0.38). Gate-tuned; 0 for dataset1
# (repeat-edge problem, popularity irrelevant).
TWPOP_BONUS = {"dataset1": 0.0, "dataset2": 0.0}
# Post-hoc CF-embedding blend. The raw driver trains and retains the Jittor
# user/item embedding before the ranker stages, then binds it through CF_EMB.
CF_BLEND = {"dataset1": 0.0, "dataset2": 0.15}
CF_EMB_PATH = {"dataset1": "cf1_full_d128.npz", "dataset2": "cf_full_d128.npz"}
# Half-life divisor for temporal-decay popularity: weight = exp(-(tmax - t) / ((tmax-tmin)/DIV)).
TWPOP_HALFLIFE_DIV = 8.0
...
class NetAttn(nn.Module):
    ...
    def execute(self, x, src, dst, hist_ids, hist_valid, hist_gap):
        ...
        any_valid = (hist_valid.sum(1) > 0).unsqueeze(1)
...
def stats_to_arrays(stats):
    ...
    if stats.pair_cnt:
        ...
        pair_keys, pair_counts, pair_lasts = pair_keys[order], pair_counts[order], pair_lasts[order]
    else:
...
if _NUMBA_OK:
    ...
    _raw_features_kernel = None


def raw_features_numba(stats, srcs, times, cands, freq, src_freq, use_src_freq):
    if not _NUMBA_OK:
        return raw_features(stats, srcs, times, cands, freq, src_freq, use_src_freq)
...
def flat_history_attn(hist_ids, hist_valid, hist_gap, ids):
    h = np.repeat(hist_ids[ids], 100, axis=0).astype(np.int32, copy=False)
    v = np.repeat(hist_valid[ids], 100, axis=0).astype(np.float32, copy=False)
    g = np.repeat(hist_gap[ids], 100, axis=0).astype(np.float32, copy=False)
    return h, v, g


def _net_forward(net, xb, sb, db, hist_data):
    """Unified forward for both Net (2-array hist) and NetAttn (3-array hist)."""
    if isinstance(net, NetAttn):
        hb, hv, hg = hist_data
        return net(xb, sb, db,
                   jt.array(hb) if not isinstance(hb, jt.Var) else hb,
                   jt.array(hv) if not isinstance(hv, jt.Var) else hv,
                   jt.array(hg) if not isinstance(hg, jt.Var) else hg)
    else:
        hb, hm = hist_data
        return net(xb, sb, db,
                   jt.array(hb) if not isinstance(hb, jt.Var) else hb,
                   jt.array(hm) if not isinstance(hm, jt.Var) else hm)


def _make_hist_data_offline(use_attn, stats, srcs, times):
    """Pre-compute full history arrays (not per-batch)."""
    if use_attn:
        return history_seq(stats, srcs, times)
    else:
        return history_ids(stats, srcs)


def _flat_hist_data(hist_data, ids):
    """Flatten a hist_data tuple (2-array or 3-array) for a batch indexed by ids."""
    if len(hist_data) == 3:
        h_ids, h_valid, h_gap = hist_data
        return flat_history_attn(h_ids, h_valid, h_gap, ids)
    else:
        h_ids, h_mask = hist_data
        return flat_history(h_ids, h_mask, ids)
...
def train_net(
    ...
    if init_net is not None:
        ...
        net = NetAttn(x.shape[-1], nodes, emb=emb)
    else:
        net = Net(x.shape[-1], nodes, emb=emb, use_hist=use_hist)
    cf_init_path = os.environ.get("CF_INIT", "")
    if cf_init_path and init_net is None:
        # Warm-start src/dst embeddings from full-graph CF embeddings (both-gate-safe injection).
        cf = np.load(cf_init_path)
        cu = cf["user"].astype(np.float32); ci = cf["item"].astype(np.float32)
    ...
    if init_net is not None:
        best_mrr = eval_mrr(net, valid_x, valid_src, valid_cand, valid_y, valid_hist_data, batch)
    ...
    for ep in range(epochs):
        ...
        for s in range(0, len(order), batch):
            ...
            hdata_b = _flat_hist_data(hist_data, ids)
            yb = jt.array(y[ids])
            score = _net_forward(net, xb, jt.array(sb), jt.array(db), hdata_b).reshape((len(ids), 100))
        ...
        mrr = eval_mrr(net, valid_x, valid_src, valid_cand, valid_y, valid_hist_data, batch)
        if mrr > best_mrr:
            best_mrr = mrr
            best_state = {k: np.asarray(v.data).copy() for k, v in net.state_dict().items()}
...
def eval_mrr(net, x, src, cand, y, hist_data, batch):
    net.eval()
    ranks = []
    for s in range(0, len(x), batch):
        ...
        sb, db = flat_ids(src, cand, ids)
        hdata_b = _flat_hist_data(hist_data, ids)
        score = np.asarray(_net_forward(net, xb, jt.array(sb), jt.array(db), hdata_b).reshape((-1, 100)).data)
...
def predict(net, x, src, cand, hist_data, batch):
    net.eval()
    out = []
    for s in range(0, len(x), batch):
        ids = np.arange(s, min(s + batch, len(x)))
...
def temporal_pop(edges, max_node):
    # Time-decayed destination popularity: recent interactions weigh more.
    # Captures short-term global trend, the dominant signal for cold-start dataset2.
    t = edges[:, 2].astype(np.float64)
    tmax = t.max()
    span = max(tmax - t.min(), 1.0)
    w = np.exp(-(tmax - t) / (span / TWPOP_HALFLIFE_DIV))
    return np.bincount(edges[:, 1].astype(np.int64), weights=w, minlength=max_node + 1).astype(np.float32)


def candidate_twpop_bonus(cand, twpop, scene):
    beta = TWPOP_BONUS.get(scene, 0.0)
    if not beta:
        return 0.0
    return beta * query_norm_score(np.log1p(twpop[cand]))


def load_cf_emb(scene):
    # Full-graph sampled-softmax user/item embeddings for the CF blend. Returns None if disabled/missing.
    w = CF_BLEND.get(scene, 0.0)
    if not w:
        return None
    path = CF_EMB_PATH.get(scene, "")
    if not path or not os.path.exists(path):
        return None
    cf = np.load(path)
    return cf["user"].astype(np.float32), cf["item"].astype(np.float32), cf["ibias"].astype(np.float32)
...
def predict_with_prop(net, x, src, cand, hist_data, batch, prop=None, alpha=0.0):
    net.eval()
    out = []
    for s in range(0, len(x), batch):
        ids = np.arange(s, min(s + batch, len(x)))
...
def model_scores_with_prop(net, x, src, cand, hist_data, batch, prop=None, alpha=0.0):
    net.eval()
    out = []
    for s in range(0, len(x), batch):
        ids = np.arange(s, min(s + batch, len(x)))
...
def save_model(path, net, mu, sd, nodes):
    model_type = "NetAttn" if isinstance(net, NetAttn) else "Net"
    jt.save({"model": net.state_dict(), "mu": mu, "sd": sd, "nodes": nodes,
              "use_hist": net.use_hist if hasattr(net, "use_hist") else True,
              "model_type": model_type}, path)


def load_model(path):
    obj = jt.load(path)
    model_type = obj.get("model_type", "Net")
    # Infer embedding dim from the checkpoint so larger-emb models load correctly.
    emb_dim = int(np.asarray(obj["model"]["src_emb.weight"].data).shape[1])
    if model_type == "NetAttn":
        net = NetAttn(len(obj["mu"]), int(obj["nodes"]), emb=emb_dim)
    else:
...
def fit_scene(
    ...
    x = raw_features_numba(train_stats, src, tim, cand, freq, src_freq, use_src_freq)
    vx = raw_features_numba(valid_stats, vsrc, vtim, vcand, freq, src_freq, use_src_freq)

    # Build history data in correct format for Net vs NetAttn
    hist_data = _make_hist_data_offline(use_attn, train_stats, src, tim)
    valid_hist_data = _make_hist_data_offline(use_attn, valid_stats, vsrc, vtim)
    ...
    print(f"{scene}: train groups={len(x)} valid={len(vx)} dim={x.shape[-1]} "
          f"attn={use_attn} hist={use_hist} seeds={n_seeds}", flush=True)
    ...
    for si in range(n_seeds):
        s = seed + si * 1000
        net = train_net(
            x, src, cand, y, hist_data,
            vx, vsrc, vcand, vy, valid_hist_data,
            epochs, batch, s, max_node + 1,
            use_hist, init_net if si == 0 else None,
            lr, loss_name, use_attn, emb,
        )
        mrr = eval_mrr(net, vx, vsrc, vcand, vy, valid_hist_data, batch)
        print(f"{scene}: seed={si} val_mrr={mrr:.5f}", flush=True)
        # Save every seed for probability-averaging ensemble at inference time.
        if n_seeds > 1:
            save_model(f"m{scene[-1]}_s{si}.pkl", net, mu, sd, max_node + 1)
        if mrr > best_mrr:
            best_mrr = mrr
            best_net = net

    save_model(f"m{scene[-1]}.pkl", best_net, mu, sd, max_node + 1)
    return float(eval_mrr(best_net, vx, vsrc, vcand, vy, valid_hist_data, batch))
    ...
    with zout.open(f"{scene}.csv", "w") as f:
        for s in range(0, len(test), batch):
            ...
            if blend < 1.0:
                ...
                x = scale(raw_features_numba(stats, src, tim, cand, freq, src_freq, use_src_freq), mu, sd)
                hist_data = _make_hist_data_offline(isinstance(base, NetAttn), stats, src, tim)
                base_score = model_scores_with_prop(
                    base, x, src, cand, hist_data,
                    max(128, batch // 4), prop, alpha,
                )
            ...
            score = score + candidate_twpop_bonus(cand, twpop, scene)
...
def _ensemble_paths(scene):
    # Ensemble members for a scene: m{d}_ens0.pkl, m{d}_ens1.pkl, ... if present, else single m{d}.pkl.
    import glob
    d = scene[-1]
...
def write_scene(zout, scene, batch, use_prop=True, use_rank=True):
    ...
    _, freq, src_freq = test_pool_freq(test, max_node)
    edges = train[["src", "dst", "time"]].to_numpy(np.int64)
    stats = Stats(edges, max_node)
    twpop = temporal_pop(edges, max_node)
    cf = load_cf_emb(scene)
    alpha = PROP_ALPHA.get(scene, 0.0) if use_prop else 0.0

    # Load ensemble members (or single model). Each contributes a query-normalized score, averaged.
    members = []
    for mp in _ensemble_paths(scene):
        ...
        if alpha:
            ...
            se = np.asarray(state["src_emb.weight"].data, dtype=np.float32)
            de = np.asarray(state["dst_emb.weight"].data, dtype=np.float32)
            ps, pd = make_prop_embeddings(edges, int(max_node) + 1, se, de)
            prop = (se, de, ps, pd)
        members.append((net, mu, sd, use_src_freq, prop))
    ...
    with zout.open(f"{scene}.csv", "w") as f:
        for s in range(0, len(test), batch):
            ...
            cand = part.iloc[:, 2:].to_numpy(np.int64, copy=False)
            acc = None
            for net, mu, sd, use_src_freq, prop in members:
                x = scale(raw_features_numba(stats, src, tim, cand, freq, src_freq, use_src_freq), mu, sd)
                hist_data = _make_hist_data_offline(isinstance(net, NetAttn), stats, src, tim)
                sc = model_scores_with_prop(net, x, src, cand, hist_data, max(128, batch // 4), prop, alpha)
...
def main():
    ...
    p.add_argument("--groups1", type=int, default=100000)
    p.add_argument("--groups2", type=int, default=150000)
    p.add_argument("--epochs", type=int, default=20)
    p.add_argument("--batch", type=int, default=512)
    p.add_argument("--seed", type=int, default=20260704)
    p.add_argument("--quick", action="store_true")
    p.add_argument("--hist", action="store_true")
    p.add_argument("--attn", action="store_true", help="use NetAttn for both datasets")
    p.add_argument("--srcfreq2", action="store_true")
    p.add_argument("--finetune", action="store_true")
    p.add_argument("--lr", type=float, default=1e-3)
    p.add_argument("--loss", choices=["ce", "pair", "mix"], default="ce")
    p.add_argument("--valid1", type=int, default=20000)
    p.add_argument("--valid2", type=int, default=30000)
    p.add_argument("--nseeds", type=int, default=1, help="number of seeds to train; best val MRR is kept")
    p.add_argument("--emb", type=int, default=64, help="embedding dimension for src/dst")
