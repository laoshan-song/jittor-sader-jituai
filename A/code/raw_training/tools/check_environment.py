#!/usr/bin/env python3
"""Fail closed before a CUDA Jittor reproduction run allocates model outputs."""
...
def main() -> None:
    ...
    if mismatch:
        raise RuntimeError("pinned dependency versions differ: " + json.dumps(mismatch, sort_keys=True))
    if not jt.has_cuda:
    ...
    if value != 14.0:
        raise RuntimeError(f"Jittor CUDA arithmetic probe differs: {value}")
    print(
        json.dumps(
            {
                "kind": "track1_jittor_cuda_environment_v1",
