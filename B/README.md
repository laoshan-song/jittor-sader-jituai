...
validates the input, tracked locked assets, and package manifest, restores the
split frozen checkpoint into the runtime directory, and generates the online
...
The repository intentionally provides both levels:

1. **Exact recorded-result reproduction**: `verify` consumes the tracked locked
   MF32 model and split frozen checkpoint. It accepts only the recorded
   `result.zip` SHA-256 and reproduces score `1.5240999401892983`.
2. **From-scratch training reproduction**: `generate-base` reads only
   `data_B.zip`, retrains the full Dataset3/Dataset4 pipeline, and emits a newly
   generated `frozen_base.ckpt`. This path exposes the complete training,
   feature, validation, fusion, and serialization method; it does not read the
   tracked locked assets and does not assert historical byte parity.
...
is executable end to end without reading a fixed base:
...
supplies the frozen-base generation chain from the official data alone. See
`code/pipeline/README.md` for details.
...
the split fixed base state, Jittor checkpoint, source code, deterministic
serializer, and integrity checks needed to generate the highest online
submission byte-for-byte. `run_verify.sh` is the authoritative exact
reproduction command; `generate-base` is the independent from-scratch training
command.
