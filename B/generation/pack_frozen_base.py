...
This is the missing upstream link of the B-list flow. The published package
only ships ``models/frozen_base.ckpt`` and reranks it with an MF32 residual;
this module produces that checkpoint from the score matrices generated by the
official-data pipeline (``reproduce_third_1.py``), so the base itself is
...
def pack(result_zip: Path, output: Path, chunk_rows: int = 4096) -> dict:
    ...
    receipt = {
        ...
        "width": WIDTH,
        "note": (
            "Regenerated frozen base. Its SHA-256 need not match the recorded "
            "locked base: missing historical checkpoints and Jittor per-machine "
            "operator perturbations make the upstream pipeline an approximate, "
