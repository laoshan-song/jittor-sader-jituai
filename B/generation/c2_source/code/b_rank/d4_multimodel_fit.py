...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.pairnew_weight_report is not None:
        ...
        weight_kind = json.loads(
            args.pairnew_weight_report.read_text(encoding="utf-8")
        ).get("kind")
        expected_valid = 60000 if weight_kind == "d4_pairnew_rank_slot_scaled_replay_transformer_v21" else 30000
        if args.valid_groups != expected_valid or args.confirm_groups != 30000:
            raise ValueError(
                f"weighted audit requires validation={expected_valid}, confirmation=30000"
            )
    ...
    if args.pairnew_transformer:
        ...
        ] or list(pairnew_transformer_jittor.DEFAULT_MEMBERS)
        if args.pairnew_weight_report is not None:
            base_kind = json.loads(
                args.pairnew_weight_report.read_text(encoding="utf-8")
            ).get("kind")
            audit = (
                pairnew_transformer_jittor.run_scaled_v21_weighted_audit
                if base_kind == "d4_pairnew_rank_slot_scaled_replay_transformer_v21"
                else pairnew_transformer_jittor.run_weighted_audit
            )
            report = audit(
                scored=scored,
                component_names=names,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="load the submitted pair-new report and audit constrained member weights",
    )
    parser.add_argument(
        "--residual-member",
        nargs=2,
