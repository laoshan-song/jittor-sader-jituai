...
from b_rank_a_port import ensemble_core as core, run
...
WIDTH = 100
WEIGHTS = np.arange(0.0, 0.3001, 0.0025)
...
def research(args: argparse.Namespace) -> dict:
    ...
    confirmation = scored["confirmation"]
    for gate in ("row_no_seen", "pair_new", "all"):
        for weight in WEIGHTS:
            candidate = apply_policy(
                confirmation["baseline"], confirmation["feature"], confirmation["seen"],
                float(weight), gate,
            )
            confirmation_selection.append({"gate": gate, "weight": float(weight), **metrics(
                confirmation["baseline"], candidate, confirmation["labels"]
            )})
    policy = max(
        selection,
...
def build(args: argparse.Namespace) -> dict:
    if sha256(args.data) != DATA_SHA256 or sha256(args.base) != V21_ZIP_SHA256:
        raise ValueError("official data or v21 base hash differs")
    ...
    output = {
        ...
        "data_sha256": DATA_SHA256,
        "base_v21_sha256": V21_ZIP_SHA256,
        ...
        "dataset4": {"rows": ROWS["dataset4"], "csv_sha256": V21_D4_SHA256},
        "source_sha256": sha256(Path(__file__).resolve()),
    }
