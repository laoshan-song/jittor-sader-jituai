#!/usr/bin/env python3
"""Train validated D4 temporal models on every split1 edge before test."""
...
def _full_split1_group(
    ...
    *,
    ...
    pool, counts = cache.history_dst_counts(cutoff)
    pool_metadata = {
        "strategy": "history",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.temporal:
        raise ValueError("at least one --temporal NAME HISTORY_SIZE SEED is required")
    specs = [(name, int(size), int(seed)) for name, size, seed in args.temporal]
    if any(size < 1 for _, size, _ in specs):
        raise ValueError("history sizes must be positive")
    if len({name for name, _, _ in specs}) != len(specs):
        raise ValueError("temporal model names must be unique")
    ...
    checkpoints = []
    for name, history_size, seed in specs:
        model, losses = temporal_attention_jittor.train_ranker(
            arrays.source,
            arrays.candidates,
            arrays.history[:, -history_size:],
            arrays.history_gap[:, -history_size:],
            arrays.labels,
        ...
        if (
            restored.source_count != history.source_vocab_size
            or restored.item_count != history.item_vocab_size
        ):
            raise ValueError(f"checkpoint vocabulary differs: {name}")
        checkpoints.append(
            {
                "name": name,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs=3,
        action="append",
        metavar=("NAME", "HISTORY_SIZE", "SEED"),
        default=[],
    )
