...
V12_BASE_ALPHA = 1.05
SCALED_V21_REPORT_SHA256 = (
    "a280855b825ee08ce64a14600646804baab9b36ac85a1c30171192be7d566ec0"
)
...
def run_scaled_v21_weighted_audit(
    *,
    scored: dict[tuple[str, str], tuple[np.ndarray, np.ndarray, np.ndarray, dict[str, np.ndarray], np.ndarray]],
    component_names: list[str],
    control_fit_path: Path,
    ...
    """Select a constrained blend against the actual online scaled-v21 D4."""
    if batch < 1:
        raise ValueError("batch must be positive")
    if _sha256(base_report_path) != SCALED_V21_REPORT_SHA256:
        raise ValueError("base pair-new report is not the online scaled-v21 report")
    base_report = json.loads(base_report_path.read_text(encoding="utf-8"))
    if (
        base_report.get("kind") != "d4_pairnew_rank_slot_scaled_replay_transformer_v21"
        or base_report.get("decision") != "PASS"
        or base_report.get("data_sha256") != EXPECTED_DATA_SHA256
        or base_report.get("component_names") != component_names
        or base_report.get("selection_replay")
        ...
        raise ValueError("base pair-new report does not satisfy scaled-v21 contract")

    control_report, indices, base_index, seen_alpha, new_alpha = _control_contract(
        control_fit_path, component_names
    )
    if base_report["control_fit"].get("sha256") != _sha256(control_fit_path):
        raise ValueError("weighted audit control report differs from scaled-v21")
    train_rows = int(base_report["training"]["requested_rows_per_replay"])
    base_alpha = float(base_report["residual_alpha"])
    if train_rows != 50000 or abs(base_alpha - 1.1) > 1e-12:
        raise ValueError("scaled-v21 selection rows or alpha differ")
    static_mean = np.asarray(base_report["training"]["static_mean"], dtype=np.float32)
    static_std = np.asarray(base_report["training"]["static_std"], dtype=np.float32)
    ...
    if set(scored) != expected_keys:
        raise ValueError("scaled-v21 audit requires exactly four replay groups")
    prepared: dict[tuple[str, str], dict[str, Any]] = {}
    for key, (scores, labels, seen, segments, static) in scored.items():
        expected_rows = 60000 if key[1] == "validation" else 30000
        if len(labels) != expected_rows or static is None:
            raise ValueError(f"scaled-v21 replay group differs: {key}")
        control = _control_score(
            scores, seen, control_report, indices, base_index, seen_alpha, new_alpha
        )
        prepared[key] = {
            "labels": labels,
            "seen": seen,
    ...
    for index, item in enumerate(candidates):
        ...
        if item["name"] == "equal6":
            ...
            alpha, _, _ = _tune_gate(
                selection_control, residual, selection_seen, selection_labels
            )
        score = _candidate_score(selection_control, residual, selection_seen, alpha)
        value = _mrr(score, selection_labels)
        target_seen = selection_seen[
            np.arange(len(selection_labels)), selection_labels
        ]
        pair_seen_mrr = _mrr(score[target_seen], selection_labels[target_seen])
        record = {
            "name": item["name"],
            "weights": item["weights"].tolist(),
            "alpha": float(alpha),
    ...
    if equal_score is None or equal_weights is None or selected is None:
        raise RuntimeError("weighted candidate search did not evaluate equal6")
    selected_record = selected[1]
    selected_score = selected[2]
    selected_weights = np.asarray(selected[3], dtype=np.float64)
    ...
    for key, values in prepared.items():
        ...
        output_split = "holdout" if split == "validation" else split
        comparisons[f"{strategy}_{output_split}"] = comparison
        expected = float(base_report["metrics"][strategy][output_split]["candidate"]["mrr"])
        baseline_errors.append(abs(expected - comparison["reference"]["mrr"]))

    def clears(record: dict[str, Any], floor: float, se_fraction: float) -> bool:
        return float(record["delta"]) >= max(floor, se_fraction * float(record["delta_se"]))

    checks = {
        "selected_non_equal_blend": selected_record["name"] != "equal6",
        "history_holdout_delta": clears(comparisons["history_holdout"], 0.0003, 0.5),
        "history_confirmation_delta": clears(comparisons["history_confirmation"], 0.0001, 0.2),
        "test_pool_holdout_delta": clears(comparisons["test_pool_holdout"], 0.0001, 0.2),
        "test_pool_confirmation_delta": clears(comparisons["test_pool_confirmation"], 0.0001, 0.2),
        "baseline_replay_reproduced": max(baseline_errors) <= 0.0002,
    }
    for name, comparison in comparisons.items():
        before = comparison["reference"]["segments"]["pair_seen"]["mrr"]
    ...
    report.update(
        {
            "kind": "d4_pairnew_rank_slot_weighted_scaled_v21_c2",
            "decision": "PASS" if all(checks.values()) else "NO_GO",
            "base_report": {
                "path": str(base_report_path),
                "sha256": _sha256(base_report_path),
            ...
            "member_weights": selected_record["weights"],
            "residual_alpha": selected_record["alpha"],
            "weighted_vs_equal6": comparisons,
            "baseline_replay_max_abs_mrr_error": max(baseline_errors),
            "checks": checks,
        }
    )
    report["architecture"]["member_blend"] = (
        "frozen constrained nonnegative weights then row normalization"
    )
    report["source_hashes"][Path(__file__).name] = _sha256(Path(__file__).resolve())
    return report
