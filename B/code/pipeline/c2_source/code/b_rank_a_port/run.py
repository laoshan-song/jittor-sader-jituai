...
PROP_ALPHA = {"dataset3": 0.08, "dataset4": 0.0}
...
RANK_BLEND = {"dataset3": 0.50, "dataset4": 0.50}
...
FREQ_BONUS = {"dataset3": 0.0, "dataset4": 0.05}
# Blend weight for the official CRAFT baseline (JittorGeometric), a genuinely different
# model family (sequence-transformer over each src's neighbor history) vs. this file's
# hand-feature MLP+MF+ranker stack. CRAFT is individually much weaker (held-out MRR
# dataset1 0.681 vs 0.848, dataset2 0.388 vs 0.554) but its errors are different enough
# to help in a blend: held-out grid search found dataset1 +0.00313 at weight 0.20,
# dataset2 +0.00238 at weight 0.05. Predictions are precomputed offline (see
# craft_baseline/main.py) and shipped as craft1.npy/craft2.npy next to this file,
# row-order-aligned with test.csv (verified via DataFrame.equals). If the .npy for a
# scene is missing, the blend is skipped entirely (falls back to pre-CRAFT behavior).
CRAFT_BLEND = {"dataset3": 0.0, "dataset4": 0.0}
_craft_cache = {}


def load_craft_scores(scene):
    if scene in _craft_cache:
        return _craft_cache[scene]
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"craft{scene[-1]}.npy")
    scores = np.load(path).astype(np.float32) if os.path.exists(path) else None
    _craft_cache[scene] = scores
    return scores


def blend_in_craft(score, scene, cand, s, batch):
    craft_w = CRAFT_BLEND.get(scene, 0.0)
    if not craft_w:
        return score
    craft_scores = load_craft_scores(scene)
    if craft_scores is None:
        return score
    craft_part = craft_scores[s : s + len(cand)]
    return (1.0 - craft_w) * query_norm_score(score) + craft_w * query_norm_score(craft_part)
...
class Stats:
    ...
    def build(self, edges):
        ordered = time_ordered_edges(edges)
        self.pair_first = {}
...
def time_ordered_edges(edges):
    """Return edges in stable time order without copying already ordered inputs."""
    if len(edges) < 2 or np.all(edges[:-1, 2] <= edges[1:, 2]):
        return edges
    return edges[np.argsort(edges[:, 2], kind="mergesort")]
...
def test_pool_freq(test, max_node, use_src_freq=True):
    ...
    if use_src_freq:
        ...
        ends = np.r_[starts[1:], len(sorted_src)]
        for start, end in zip(starts, ends):
            u = int(sorted_src[start])
            xs = cand[order[start:end]].ravel()
            uniq, cnt = np.unique(xs, return_counts=True)
            for v, c in zip(uniq[cnt > 1], cnt[cnt > 1]):
                src_freq[u * BASE + int(v)] = np.log1p(float(c))
...
def split_edges(scene, train):
    ...
    if "split" in train.columns:
        ...
        fraction = float(os.environ.get("D1_HIST_FRAC", "0.80"))
        cut = int(len(arr) * fraction)
        end = min(len(arr), cut + int(len(arr) * 0.20))
        hist, target = arr[:cut], arr[cut:end]
    segments = split1_segments(target)
    return hist, segments["base_train"], segments["base_valid"]


def prepare_training_scene(scene, use_srcfreq2=False):
    """Build the immutable data/statistics shared by one base/ranker member."""
    print(f"{scene}: read", flush=True)
    train, test = read_scene(scene)
    max_node = node_max(train, test)
    use_src_freq = scene == "dataset3" or (scene == "dataset4" and use_srcfreq2)
    pool, freq, src_freq = test_pool_freq(test, max_node, use_src_freq)
...
def confirmation_edges(train):
    """Return the untouched final split1 time segment."""
    cols = ["src", "dst", "time"]
    target = train[train["split"] == 1][cols].to_numpy(np.int64)
    target = target[np.argsort(target[:, 2], kind="mergesort")]
    return split1_segments(target)["confirmation"]


def split1_segments(target):
    """Split ordered split1 edges into causal base/meta/evaluation segments."""
    ...
    if fractions.shape != (5,) or np.any(fractions <= 0.0):
        raise ValueError("B_SEGMENT_FRACTIONS must contain five positive values")
    fractions /= fractions.sum()
    boundaries = np.rint(np.cumsum(fractions) * len(target)).astype(np.int64)
    boundaries[-1] = len(target)
    if np.any(np.diff(np.r_[0, boundaries]) <= 0):
        raise ValueError("split1 is too small for five non-empty segments")
    names = ("base_train", "base_valid", "meta_train", "validation", "confirmation")
    starts = np.r_[0, boundaries[:-1]]
    return {
        name: target[int(start):int(end)]
        for name, start, end in zip(names, starts, boundaries)
    }
...
def _cf_hash_sign(value, dimension):
    x = np.uint64(value) + np.uint64(dimension + 1) * np.uint64(0x9E3779B97F4A7C15)
    x ^= x >> np.uint64(30)
    x *= np.uint64(0xBF58476D1CE4E5B9)
    x ^= x >> np.uint64(27)
    x *= np.uint64(0x94D049BB133111EB)
    x ^= x >> np.uint64(31)
    return 1.0 if (x & np.uint64(1)) else -1.0


@numba.njit(cache=True, parallel=True)
def _neighbor_sketch(neighbors, lengths, dimension, output):
    for node in numba.prange(len(lengths)):
        length = lengths[node]
        if length:
            for index in range(length):
                value = neighbors[node, index]
                for feature in range(dimension):
                    output[node, feature] += _cf_hash_sign(value, feature)


@numba.njit(cache=True, parallel=True)
def _aggregate_neighbor_sketch(neighbors, lengths, signatures, output):
    for node in numba.prange(len(lengths)):
        length = lengths[node]
        if length:
            for index in range(length):
                neighbor = neighbors[node, index]
                for feature in range(signatures.shape[1]):
                    output[node, feature] += signatures[neighbor, feature]


@numba.njit(cache=True, parallel=True)
def _bipartite_sketch_cf_kernel(
    source,
    candidates,
    source_signature,
    ...
    dimension = source_signature.shape[1]
    for row in numba.prange(len(source)):
        u = source[row]
        for column in range(candidates.shape[1]):
            v = candidates[row, column]
            item_dot = 0.0
            user_dot = 0.0
            if u <= max_node and v <= max_node:
                for feature in range(dimension):
                    a = source_profile[u, feature]
                    b = destination_signature[v, feature]
                    c = source_signature[u, feature]
                    d = destination_profile[v, feature]
                    item_dot += a * b
                    user_dot += c * d
                item_dot /= dimension
                user_dot /= dimension
                item_overlap = max(item_dot, 0.0)
                user_overlap = max(user_dot, 0.0)
                item_norm = item_overlap / (
                    (source_degree[u] + 1.0) * np.sqrt(destination_degree[v] + 1.0)
                )
                user_norm = user_overlap / (
                    (destination_degree[v] + 1.0) * np.sqrt(source_degree[u] + 1.0)
                )
            else:
                item_overlap = 0.0
                user_overlap = 0.0
                item_norm = 0.0
                user_norm = 0.0
            output[row, column, 0] = np.log1p(item_overlap)
            output[row, column, 1] = item_norm
            output[row, column, 2] = np.log1p(user_overlap)
            output[row, column, 3] = user_norm
            output[row, column, 4] = np.log1p(item_overlap + user_overlap)
...
def _cf_sketches(stats, dimension=64):
    cached = getattr(stats, "_cf_sketches", None)
    if cached is None or cached[0] != dimension:
        out_sorted, out_len, in_sorted, in_len = _cf_sorted_sets(stats)
        source_signature = np.zeros((len(out_len), dimension), np.float32)
        destination_signature = np.zeros((len(in_len), dimension), np.float32)
        _neighbor_sketch(out_sorted, out_len, dimension, source_signature)
        _neighbor_sketch(in_sorted, in_len, dimension, destination_signature)
        source_profile = np.zeros_like(source_signature)
        destination_profile = np.zeros_like(destination_signature)
        _aggregate_neighbor_sketch(out_sorted, out_len, destination_signature, source_profile)
        _aggregate_neighbor_sketch(in_sorted, in_len, source_signature, destination_profile)
        cached = (dimension,
            source_signature,
            destination_signature,
            source_profile,
        ...
        stats._cf_sketches = cached
    return cached[1:]


def raw_features_numba_v3(stats, srcs, times, cands, freq, src_freq, use_src_freq):
    # v3 = v1's 22/24 features + 5 CF features. Shared-ID D3 keeps the exact
    # neighbor-overlap kernel; large bipartite D4 uses fixed-width two-sided
    # sketches so feature cost stays linear in query count. Motivation (measured
    ...
    if use_src_freq:
        ...
        _bipartite_cf_kernel(
            ...
            in_sorted,
            in_len,
            np.int64(stats.max_node),
            cf,
        )
    else:
        _, out_len, _, in_len = _cf_sorted_sets(stats)
        source_signature, destination_signature, source_profile, destination_profile = _cf_sketches(stats)
        _bipartite_sketch_cf_kernel(
            srcs.astype(np.int64),
            cands.astype(np.int64),
            source_signature,
            destination_signature,
...
@numba.njit(cache=True, parallel=True)
def _tie_rank_kernel(values, output):
    columns = values.shape[1]
    denominator = max(columns - 1, 1)
    for row in numba.prange(values.shape[0]):
        for feature in range(values.shape[2]):
            order = np.argsort(values[row, :, feature])
            start = 0
            while start < columns:
                stop = start + 1
                value = values[row, order[start], feature]
                while stop < columns and values[row, order[stop], feature] == value:
                    stop += 1
                percentile = ((start + stop - 1) * 0.5) / denominator
                for index in range(start, stop):
                    output[row, order[index], feature] = percentile
                start = stop
...
def add_query_norm(raw):
    ...
    _tie_rank_kernel(raw.astype(np.float32, copy=False), rank)
...
def train_net(
    ...
    set_seed(seed)
    rng = np.random.default_rng(seed)
    net = init_net if init_net is not None else Net(x.shape[-1], nodes, use_hist=use_hist)
...
def _accumulate_prop_embeddings(edges, src_emb, dst_emb, prop_src, prop_dst, src_deg, dst_deg):
    for index in range(len(edges)):
        u = edges[index, 0]
        v = edges[index, 1]
        for dim in range(src_emb.shape[1]):
            prop_src[u, dim] += dst_emb[v, dim]
            prop_dst[v, dim] += src_emb[u, dim]
        src_deg[u] += 1.0
        dst_deg[v] += 1.0
...
def make_prop_embeddings(edges, nodes, src_emb, dst_emb):
    ...
    _accumulate_prop_embeddings(
        time_ordered_edges(edges), src_emb, dst_emb, prop_src, prop_dst, src_deg, dst_deg
    )
...
def base_score_extra_features(base_score):
    ...
    values = base_score[:, :, None].astype(np.float32, copy=False)
    rank = np.empty_like(values)
    _tie_rank_kernel(values, rank)
    return np.stack([q, rank[:, :, 0]], axis=2).astype(np.float32)
...
def make_fast_context(edges, max_node, src_emb, dst_emb, src_freq, stats=None):
    ...
    if stats is None:
        ...
        arrays = stats.get_arrays()
        keys = arrays["pair_keys"]
        cnt = arrays["pair_counts"]
        last = arrays["pair_lasts"]
...
def load_model(path):
    obj = jt.load(path)
    net = Net(len(obj["mu"]), int(obj["nodes"]), use_hist=bool(obj.get("use_hist", False)))
    net.load_state_dict(obj["model"])
...
def fit_scene(
    ...
    set_seed(seed)
    if prepared is None:
        prepared = prepare_training_scene(scene, use_srcfreq2)
    if prepared["scene"] != scene:
        raise ValueError("prepared scene does not match")
    max_node = prepared["max_node"]
    use_src_freq = prepared["use_src_freq"]
    pool, freq, src_freq = prepared["pool"], prepared["freq"], prepared["src_freq"]
    hist, tr_pos, va_pos = prepared["hist"], prepared["tr_pos"], prepared["va_pos"]
    train_stats, valid_stats = prepared["train_stats"], prepared["valid_stats"]
...
def train_fast_scene(scene, groups, valid_groups, epochs, batch, seed, prepared=None):
    ...
    set_seed(seed)
    if prepared is None:
        prepared = prepare_training_scene(scene)
    if prepared["scene"] != scene:
        raise ValueError("prepared scene does not match")
    max_node = prepared["max_node"]
    pool, freq, src_freq = prepared["pool"], prepared["freq"], prepared["src_freq"]
    ...
    train_ctx = make_fast_context(hist, max_node, src_emb, dst_emb, src_freq, train_stats)
    x = fast_rank_features(train_ctx, train_stats, src, tim, cand, freq)
    valid_edges = prepared["valid_edges"]
    valid_stats = prepared["valid_stats"]
    valid_ctx = make_fast_context(valid_edges, max_node, src_emb, dst_emb, src_freq, valid_stats)
...
def write_fast_scene(zout, scene, batch):
    ...
    _, freq, src_freq = test_pool_freq(test, max_node, use_src_freq)
    ...
    ctx = make_fast_context(edges, max_node, src_emb, dst_emb, src_freq, stats)
    prop = None
    ...
    with zout.open(f"{scene}.csv", "w") as f:
        for s in range(0, len(test), batch):
            ...
            score = blend_in_craft(score, scene, cand, s, batch)
...
def write_scene(zout, scene, batch, use_prop=True, use_rank=True):
    ...
    _, freq, src_freq = test_pool_freq(test, max_node, use_src_freq)
    stats = Stats(train[["src", "dst", "time"]].to_numpy(np.int64), max_node)
    net, mu, sd = load_model(f"m{scene[-1]}.pkl")
    feat_fn, use_src_freq = feat_fn_for_dim(len(mu))
    prop = None
    ...
    if alpha:
        ...
        src_emb = np.asarray(state["src_emb.weight"].data, dtype=np.float32)
        dst_emb = np.asarray(state["dst_emb.weight"].data, dtype=np.float32)
        prop_src, prop_dst = make_prop_embeddings(train[["src", "dst", "time"]].to_numpy(np.int64), int(max_node) + 1, src_emb, dst_emb)
    ...
    with zout.open(f"{scene}.csv", "w") as f:
        for s in range(0, len(test), batch):
            ...
            x = scale(feat_fn(stats, src, tim, cand, freq, src_freq, use_src_freq), mu, sd)
            hist_ids, hist_mask = history_ids(stats, src)
            score = model_scores_with_prop(net, x, src, cand, hist_ids, hist_mask, max(128, batch // 4), prop, alpha)
            score = score + candidate_freq_bonus(cand, freq, scene)
            score = blend_in_craft(score, scene, cand, s, batch)
            prob = scores_to_prob(score)
            text = "".join(",".join(f"{p:.8f}" for p in row) + "\n" for row in prob)
            f.write(text.encode())
            print(f"{scene}: wrote {min(s + batch, len(test))}/{len(test)}", flush=True)


def submit(batch, use_prop=True, use_rank=True):
    with zipfile.ZipFile("result.zip", "w", zipfile.ZIP_DEFLATED) as zout:
        write_scene(zout, "dataset3", batch, use_prop, use_rank)
...
def main():
    ...
    p.add_argument("--rank-scene", choices=["dataset3", "dataset4", "both"], default="dataset4")
    ...
    if args.mode in ("all", "train"):
        ...
        print(f"valid dataset3={s1:.6f} dataset4={s2:.6f} sum={s1 + s2:.6f}", flush=True)
    if args.mode == "rank":
        scenes = ["dataset3", "dataset4"] if args.rank_scene == "both" else [args.rank_scene]
        for scene in scenes:
            train_fast_scene(
                scene,
                args.rank_groups,
                ...
                args.rank_epochs,
                args.batch,
                args.seed + (11 if scene == "dataset3" else 12),
            )
