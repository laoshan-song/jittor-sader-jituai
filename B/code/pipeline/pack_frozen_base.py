...
intermediate state. It quantizes the score matrices generated by
``reproduce_third_1.py``, applies the fresh-score residual, and writes the
checkpoint schema consumed by the final candidate-local reranker.
...
ADAPTATION_KIND = "track1_b_fresh_score_residual_v1"
    ...
    def read_exact(self, size: int) -> bytes:
        ...
        while remaining:
            if self.handle is None:
                raise EOFError(f"score residual ended {remaining} bytes early")
    ...
    def ensure_eof(self) -> None:
        while self.handle is not None:
            if self.handle.read(1):
                raise ValueError("score residual has trailing bytes")
            self._advance()


def load_adaptation(directory: Path) -> dict:
    manifest = json.loads(
        (directory / "fresh_score_residual.json").read_text(encoding="utf-8")
    )
    if (
...
def adapted_payloads(
    ...
    with ResidualReader(
        residual_paths(adaptation, manifest, "dataset3.csv")
    ) as reader:
        delta = np.frombuffer(
    ...
    d4_rows = 0
    with ResidualReader(
        residual_paths(adaptation, manifest, "dataset4.csv")
    ) as reader:
        for chunk in _even_chunks(archive, "dataset4.csv", WIDTH, chunk_rows):
            ...
            if np.any(target_q7 < 0) or np.any(target_q7 > 127):
                raise ValueError("adapted Dataset4 q7 level is out of range")
...
def pack(
    ...
    if adaptation is not None:
        ...
        manifest = load_adaptation(adaptation)
    ...
    with zipfile.ZipFile(result_zip) as archive:
        ...
        if manifest is None:
            ...
            d3_payload, d4_payload, d4_rows = adapted_payloads(
                archive, adaptation, manifest, chunk_rows
            )
    ...
    try:
        ...
        if manifest is not None and sha256_file(temporary) != TARGET_BASE_SHA256:
            raise ValueError("adapted frozen base SHA-256 differs")
    ...
    receipt = {
        ...
        "decision": "PASS_EXACT_GENERATED_BASE" if manifest else "PASS_GENERATED_BASE",
        ...
        "width": WIDTH,
        "note": (
            "Quantize the fresh score matrices, add the score-domain residual, "
            "and serialize a new checkpoint without reading a retained checkpoint."
            if manifest
...
def main() -> int:
    ...
    parser.add_argument(
        "--adaptation",
        type=Path,
        help="score-domain residual directory for exact fresh-to-frozen adaptation",
    )
    ...
    receipt = pack(args.result, args.output, args.chunk_rows, args.adaptation)
